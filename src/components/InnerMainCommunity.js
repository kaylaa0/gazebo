import "../style/InnerMainCommunity.css";
...
const InnerMainCommunityContext = React.createContext();

class InnerMainCommunity extends React.Component {
  testContent = {
    1: {
      communityID: 1,
  ...
  render() {
    return (
      <InnerMainCommunityContext.Provider value={this.state.communityName}>
        <Link
          class="nav-link-ghost"
          to={`/communityPage/${this.state.communityName}`}
        >
          <div
            className="InnerMainCommunityCard"
            style={{
              display: "flex",
          ...
          >
            <div className="ClubBadgesDiv">
              <div className="ClubBadgesDivInner"></div>
            </div>
            <div
              className="ClubNameDiv"
              style={{
                backgroundColor: `${this.state.communityColor}`,
              }}
            >
              <InnerMainCommunityContext.Consumer>
                {(communityName) =>
                  communityName ? communityName : "communityName invalid"
                }
              </InnerMainCommunityContext.Consumer>
            </div>
          </div>
        </Link>
      </InnerMainCommunityContext.Provider>
    );
  }
}

export default InnerMainCommunity;
